...
const MODEL_URL = `${process.env.PUBLIC_URL}/models/cube.glb`;
useGLTF.preload(MODEL_URL);

const TARGET_SIZE = 2.8; // max dimension after normalisation

// Object 4 - The Five Critical Systems. Low-poly cube whose parts lock together
// into one connected structure — the five systems forming a single whole. Plays
// its bundled clip on loop, with the shared gentle sway + focus lift + fade.
export default function CubeSystems({ index = 4 }) {
  ...
  useFrame((state) => {
    ...
    g.rotation.y = Math.sin(t * 0.25) * 0.2 * rm;
    g.scale.setScalar(0.96 + f * 0.04);
  });

  return (
    <group ref={group} position={OBJECT_POSITIONS.pillars} {...hoverProps("pillars")}>
